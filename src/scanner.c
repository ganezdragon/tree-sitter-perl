...
bool handle_interpolation(Scanner *scanner, TSLexer *lexer,
...
void set_end_delimiter(Scanner *scanner) {
...
bool parse_start_delimiter(Scanner *scanner, TSLexer *lexer,
...
bool handle_escape_sequence(TSLexer *lexer, enum TokenType surrounding_token) {
...
bool scan_nested_delimiters(Scanner *scanner, TSLexer *lexer,
                            enum TokenType token_type) {
    while (lexer->lookahead) {
        if (lexer->lookahead == scanner->end_delimiter) {
            ...
            lexer->mark_end(lexer);
            return true;
        } else if (lexer->lookahead == scanner->start_delimiter) {
            lexer->result_symbol = token_type;
            advance(lexer);
...
bool parse_delimited_and_interpolated_content(Scanner *scanner, TSLexer *lexer,
                                              enum TokenType token_type,
...
bool process_separator_delimiter(Scanner *scanner, TSLexer *lexer,
